...
public class prueba_del_metodo_dificil {
    ...
    public void borrar_de_pila1() {
      /*Método que borre un elemento de la PILA, con la premisa que si es el
       del tope(último elemento insertado)
simplemente se borra y se manda a la COLA2 (SE PUEDE IMPRIMIR SI LO DESEAN) ,
 si no es el del tope, sacar elemento de la PILA y
 pasarlo a otra clase PILA TEMPORAL hasta eliminar el elemento indicado, después regresar de la PILATEMPORAL a la PILA.*/
        System.out.println("Dato que quieres borrar");
        int dato=s.nextInt();
        if(pila.peek()==dato){
            cola2.add(pila.pop());
        }else {
    ...
    public static void main(String[] args) {
        Scanner s = new Scanner(System.in);
        prueba_del_metodo_dificil p = new prueba_del_metodo_dificil();
        int opc;
        do {
